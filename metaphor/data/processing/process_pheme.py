...
def process_annotation(annotation: Dict[str, str]):
    ...
    if 'misinformation' in annotation.keys() and 'true' in annotation.keys():
        if int(annotation['misinformation']) == 0 and int(
                annotation['true']) == 0:
            label = "unverified"
        elif int(annotation['misinformation']) == 0 and int(
                annotation['true']) == 1:
            label = "true"
        elif int(annotation['misinformation']) == 1 and int(
                annotation['true']) == 0:
            label = "false"
        elif int(annotation['misinformation']) == 1 and int(
                annotation['true']) == 1:
...
def get_tweet_information(annotation_path: PosixPath):
    ...
    source_tweet_path = os.path.join("/".join(split_path[:-1]),
                                     'source-tweets', f"{tweet_id}.json")
...
def main(root, output_path):
    ...
    output_writer.writerow(
        ["tweet_id", "timestamp", "topic", "text", "veracity"])
